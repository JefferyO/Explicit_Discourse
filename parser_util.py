        ...
        if '..' in conn:
            ...
            if c1_indice!= [] and c2_indice != []:
                if c1_indice[0][0] < c2_indice[0][0]:
                    temp = set(list(t for t in (list(c1_indice[0])+list(c2_indice[0]))))
                    if tagged & temp == set([]):
                        indices.append(list(c1_indice[0])+list(c2_indice[0]))# [[7], [10]]
...
def put_feature_to_model_connective(feature_path, model_path, model_output_path):
    cmd = config.MALLET_PATH + "/bin/mallet classify-file --input " + feature_path + " --output " + model_output_path
    os.system(cmd)
